...
package fr.epita.rabincore.test;
...
public class TestHibernateDAO {
	...
	public void testCreateThroughsave() throws SQLException {
		...
		// THEN
		final int count = executeQuery("SELECT count(*)  FROM IDENTITIES WHERE IDENTITIES.IDENTITY_EMAIL='rabin@ojha.com'");
	...
	public void testdeleteThroughdelete() throws SQLException {
		...
		//THEN
		dao.delete(identity);
		final int count1 = executeQuery("SELECT count(*)  FROM IDENTITIES WHERE IDENTITIES.IDENTITY_EMAIL='rabin@ojha.com'");
	...
	public void testsearchThroughsearch() throws SQLException {
		LOGGER.info("beginning test");
		//GIVEN
		final Identity identity=new Identity(0, "rabin","rabin@ojha.com" ,new Date());
